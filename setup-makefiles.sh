...
RAMDIR=device/$VENDOR/$DEVICE/ramdisk
...
echo >> $MAKEFILE
echo "#vendor ramdisk files" >> $MAKEFILE
echo "PRODUCT_COPY_FILES += \\" >> $MAKEFILE
LINEEND=" \\"
COUNT=`cat ramdisk-files.txt | grep -v ^# | grep -v ^$ | wc -l | awk {'print $1'}`
for FILE in `cat ramdisk-files.txt | grep -v ^# | grep -v ^$`; do
    COUNT=`expr $COUNT - 1`
    if [ $COUNT = "0" ]; then
        LINEEND=""
    fi
